...
app.get('/api/ext-transaction-pdf', async (req, res) => {
    try {
        ...
        const apiUrl = `http://ime.dubaicustoms.network/api/ext-transaction-pdf/?start_date=${startDate}&end_date=${endDate}`;
...
app.post('/api/send-message-email', async (req, res) => {
    try {
        ...
        const response = await fetch('http://ime.dubaicustoms.network/api/send-message-email/', {
            method: 'POST',
            headers: {
                'Authorization': authToken,
                'Content-Type': 'application/json',
...
/////////////////////////////////INTRANET/////////////////////////////////////////////
app.post('/api/ext-verify-wallet-address', async (req, res) => {
    try {
        ...
        // Prepare the body to forward to the external API
        const requestBody = req.body;

        // Send the request to the external API
        const response = await fetch('https://intranet.dubaicustoms.network/api/ext-verify-wallet-address', {
            method: 'POST',
            headers: {
                'Authorization': authToken,    // Include the Bearer token
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(requestBody) // Forward the body from the incoming request
        });
        ...
        res.status(500).json({ error: 'Internal server error during wallet address verification' });
    }
});
// Proxy route for service-data (inspection today)
app.get('/api/service-data', async (req, res) => {
    try {
        const { service_name, filter, user_id } = req.query;

        if (!service_name || !filter || !user_id) {
            return res.status(400).json({ error: 'Missing required query parameters: service_name, filter, or user_id' });
        }

        const apiUrl = `https://intranet.dubaicustoms.network/api/service-data/?service_name=${encodeURIComponent(service_name)}&filter=${encodeURIComponent(filter)}&user_id=${encodeURIComponent(user_id)}`;
        ...
        res.status(500).json({ error: 'Internal server error during service data fetch' });
    }
});


// Proxy route for service-details
app.get('/api/service-details', async (req, res) => {
    try {
        const { service_name, user_id } = req.query;

        if (!service_name || !user_id) {
            return res.status(400).json({ error: 'Missing required query parameters: service_name or user_id' });
        }

        const apiUrl = `https://intranet.dubaicustoms.network/api/service-details/?service_name=${encodeURIComponent(service_name)}&user_id=${encodeURIComponent(user_id)}`;
        ...
        res.status(500).json({ error: 'Internal server error during service details fetch' });
    }
});

// Proxy route for fetch_urls
app.post('/api/fetch_urls', async (req, res) => {
    try {
        const { token, serviceName } = req.body;

        if (!token || !serviceName) {
            return res.status(400).json({ error: 'Missing required fields: token or serviceName' });
        }

        const response = await fetch('https://intranet.dubaicustoms.network/api/fetch_urls/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            if (data.redirect_url) {
                ...
                // If no redirect_url, just send the normal response
                res.status(200).json(data);
            }
        } else {
            if (response.headers.get('content-type')?.includes('application/json')) {
        ...
        console.error('Error during fetch_urls:', error);
        res.status(500).json({ error: 'Internal server error during fetch_urls' });
    }
});
